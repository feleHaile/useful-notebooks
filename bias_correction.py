...
class bias_correction(object):
    ...
    def correct(self, method = 'modified_quantile', lower_limit=0.1, cdf_threshold=0.9999999):
        if method == 'gamma_mapping':
            ...
            corrected = normal_correction(self.obs_data, self.mod_data, self.sce_data, lower_limit=lower_limit, \
                                                      cdf_threshold=cdf_threshold)
...
model = bias_correction(ref, ctr, data)
mod = model.correct(method = 'normal_mapping')
mod.corr(ref)
