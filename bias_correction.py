...
class bias_correction(object):
    ...
    def correct(self, method = 'modified_quantile', \
        ...
        if method == 'gamma_mapping':
            corrected = bias_correction.gamma_correction(self.obs_data, self.mod_data, \
                                         self.sce_data, lower_limit=lower_limit, \
                                         cdf_threshold=cdf_threshold)
        elif method == 'normal_mapping':
            corrected = bias_correction.normal_correction(self.obs_data, self.mod_data, \
                                          self.sce_data, lower_limit=lower_limit, \
                                          cdf_threshold=cdf_threshold)
        elif method == 'basic_quantile':
            corrected = bias_correction.quantile_correction(self.obs_data, self.mod_data, \
                                            self.sce_data, modified = False)
        else:
            corrected = bias_correction.quantile_correction(self.obs_data, self.mod_data, \
                                            self.sce_data, modified = True)
    ...
    def quantile_correction(obs_data, mod_data, sce_data, modified = True):
        cdf = ECDF(mod_data)
        p = cdf(sce_data)
        cor = np.subtract(*[np.nanquantile(x, p) for x in [obs_data, mod_data]])
        mid = np.subtract(*[np.nanquantile(x, 0.5) for x in [obs_data, mod_data]])
        g = np.true_divide(*[np.nanquantile(x, 0.5) for x in [obs_data, mod_data]])

        iqr_obs_data = np.subtract(*np.nanquantile(obs_data, [0.75, 0.25]))
        iqr_mod_data = np.subtract(*np.nanquantile(mod_data, [0.75, 0.25])) 
    ...
    def gamma_correction(obs_data, mod_data, sce_data, lower_limit=0.1, cdf_threshold=0.9999999):
        obs_raindays, mod_raindays, sce_raindays = [x[x >= lower_limit] \
                                                    for x in [obs_data, mod_data, sce_data]]
        obs_gamma, mod_gamma, sce_gamma = [gamma.fit(x) \
                                           for x in [obs_raindays, mod_raindays, sce_raindays]]   
        ...
        obs_inverse, mod_inverse, sce_inverse = [1. / (1. - x) for x in [obs_cdf_intpol, \
                                                                        mod_cdf_intpol, sce_cdf]]

        adapted_cdf = 1 - 1. / (obs_inverse * sce_inverse / mod_inverse)
        adapted_cdf[adapted_cdf < 0.] = 0.

        initial = gamma.ppf(np.sort(adapted_cdf), *obs_gamma) * \
                  gamma.ppf(sce_cdf, *sce_gamma) / gamma.ppf(sce_cdf, *mod_gamma)
    ...
    @staticmethod
    def normal_correction(obs_data, mod_data, sce_data, lower_limit=0.1, cdf_threshold=0.9999999):
        ...
        mod_cdf_intpol = np.interp(np.linspace(1, mod_len, sce_len),
                                   ...
                                   mod_cdf)
        obs_cdf_shift, mod_cdf_shift, sce_cdf_shift = [(x - 0.5) \
                                                       for x in [obs_cdf_intpol, mod_cdf_intpol, sce_cdf]]

        obs_inverse, mod_inverse, sce_inverse = [1. / (.5 - np.abs(x)) \
                                                 for x in [obs_cdf_shift, mod_cdf_shift, sce_cdf_shift]]
